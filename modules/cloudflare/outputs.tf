...
output "managed_resource_counts" {
  ...
  value = {
    dns_records           = length(cloudflare_dns_record.records)
    rulesets              = length(cloudflare_ruleset.zone_rulesets)
    access_applications   = length(cloudflare_zero_trust_access_application.apps)
    access_policies       = length(cloudflare_zero_trust_access_policy.policies)
    worker_scripts        = length(cloudflare_workers_script.scripts)
    worker_routes         = length(cloudflare_workers_route.routes)
